...
fn collect_subgraphs(
    document: &Document,
) -> Result<HashMap<String, SupergraphRoute>, SupergraphError> {
    let mut out = HashMap::new();
    for def in &document.definitions {
        let Definition::EnumTypeDefinition(enum_def) = def else {
            continue;
        };
    ...
    if out.is_empty() {
        return Err(SupergraphError::Validation(
            "supergraph has no `enum join__Graph` declaration; was the SDL composed by Apollo?"
                .into(),
        ));
    }
    Ok(out)
...
fn parse_field_set(field_set: &str) -> Vec<String> {
    field_set.split_whitespace().map(|s| s.to_owned()).collect()
}
