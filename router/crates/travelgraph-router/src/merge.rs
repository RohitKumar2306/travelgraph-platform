...
pub fn merge(field_results: Vec<FieldFetchResult>) -> MergedResponse {
    ...
    for FieldFetchResult { field, initial, entities } in field_results {
...
mod tests {
    ...
    fn merges_initial_and_one_entity_fetch_by_index() {
        ...
        let entity_result = EntityFetchResult {
            ...
            representations: vec![
                EntityRef { typename: "Property".into(), key_field: "id".into(), key_value: Value::String("1".into()) },
                EntityRef { typename: "Property".into(), key_field: "id".into(), key_value: Value::String("2".into()) },
            ],
            outcome: Ok(GraphQLResponse {
    ...
    fn extender_subgraph_failure_emits_error_but_keeps_initial_data() {
        ...
        let entity_result = EntityFetchResult {
            plan: EntityFetch {
                ...
                key_field: "id".into(),
                fragment_body: "price { totalAmount }".into(),
                selection_path: SelectionPath { root_key: "searchProperties".into(), is_list: true },
            },
        ...
        let data = merged.response.data.unwrap();
        let arr = data["searchProperties"].as_array().unwrap();
        assert_eq!(arr[0]["name"], "Alpha", "initial fields preserved on extender failure");
