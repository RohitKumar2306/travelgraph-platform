...
use apollo_compiler::ast::{
    Definition, Document, OperationDefinition, OperationType, Selection, Value, VariableDefinition,
};
...
fn partition_for_entity(
    ...
    for selection in &field.selection_set {
        let Selection::Field(sub) = selection else {
            continue;
        };
...
"#;
    ...
    fn single_subgraph_query_produces_single_subgraph_plan() {
        ...
        assert!(
            f.entity_fetches.is_empty(),
            "single-subgraph queries must not generate entity fetches"
        );
    }
    ...
    fn property_with_reviews_plans_initial_then_entity_fetch() {
        ...
        assert!(
            f.initial.query_text.contains(" id "),
            "id should be projected: {}",
            f.initial.query_text
        );
    ...
    fn search_with_price_and_reviews_plans_two_extender_fetches() {
        ...
        let f = &plan.field_fetches[0];
        assert_eq!(f.initial.subgraph, "property");
        let extenders: Vec<_> = f
            .entity_fetches
            .iter()
            .map(|e| e.subgraph.as_str())
            .collect();
        assert!(extenders.contains(&"pricing"));
        assert!(extenders.contains(&"review"));
        assert_eq!(
            f.entity_fetches.len(),
            2,
            "one batched _entities call per extending subgraph"
        );
    }
