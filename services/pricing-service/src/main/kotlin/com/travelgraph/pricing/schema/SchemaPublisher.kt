package com.travelgraph.pricing.schema
...
class SchemaPublisher(
    ...
    @Value("\${SCHEMA_OWNER_TEAM:pricing-platform}") private val ownerTeam: String,
        ...
        repeat(30) { attempt ->
            val published = runCatching {
                ...
                webClient.post()
                    ...
                    .bodyValue(mapOf("serviceName" to "pricing-service", "version" to Instant.now().toString(), "ownerTeam" to ownerTeam, "sdl" to sdl))
                    .retrieve()
                    .toBodilessEntity()
                    .block()
                true
            }.getOrElse {
                if (attempt == 29) println("schema publication failed for pricing-service: ${it.message}")
                false
            }
            if (published) return
            Thread.sleep(1_000)
