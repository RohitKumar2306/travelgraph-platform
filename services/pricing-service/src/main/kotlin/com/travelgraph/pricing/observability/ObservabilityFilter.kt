...
class ObservabilityFilter(
    ...
    override fun filter(exchange: ServerWebExchange, chain: WebFilterChain): Mono<Void> {
        ...
        val context = ObservabilityContext(
            exchange.request.headers.getFirst("apollographql-client-name") ?: "__anonymous__",
            exchange.request.headers.getFirst("x-operation-name") ?: "<anonymous>",
        ...
        return chain.filter(exchange).doFinally {
            ...
            MDC.remove("user_id")
            meterRegistry.counter("pricing_requests_total", "client_name", context.clientName, "operation_name", context.operationName, "status", exchange.response.statusCode?.value()?.toString() ?: "200").increment()
        }
    }
}
