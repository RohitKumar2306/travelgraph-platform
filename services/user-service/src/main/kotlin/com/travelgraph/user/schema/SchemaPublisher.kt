package com.travelgraph.user.schema
...
class SchemaPublisher(
    ...
    @Value("\${SCHEMA_OWNER_TEAM:user-platform}") private val ownerTeam: String,
        ...
        repeat(30) { attempt ->
            val published = runCatching {
                ...
                webClient.post()
                    ...
                    .bodyValue(mapOf("serviceName" to "user-service", "version" to Instant.now().toString(), "ownerTeam" to ownerTeam, "sdl" to sdl))
                    .retrieve()
                    .toBodilessEntity()
                    .block()
                true
            }.getOrElse {
                if (attempt == 29) println("schema publication failed for user-service: ${it.message}")
                false
            }
            if (published) return
            Thread.sleep(1_000)
