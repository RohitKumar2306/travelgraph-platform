...
class ObservabilityFilter(private val meterRegistry: MeterRegistry, private val contextHolder: ObservabilityContextHolder) : WebFilter {
    override fun filter(exchange: ServerWebExchange, chain: WebFilterChain): Mono<Void> {
        ...
        return chain.filter(exchange).doFinally {
            ...
            MDC.remove("user_id")
            meterRegistry.counter("review_requests_total", "client_name", context.clientName, "operation_name", context.operationName, "status", exchange.response.statusCode?.value()?.toString() ?: "200").increment()
        }
    }
}
